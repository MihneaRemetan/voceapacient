...
export const createPost = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (!body || body.trim().length < 30) {
      res.status(400).json({
        error: 'Descrierea trebuie să aibă minimum 30 de caractere.'
      });
      return;
    }
    ...
    if (!unitName || !locality || !county) {
      res.status(400).json({
        error: 'Spitalul, localitatea și județul sunt obligatorii.'
      });
      return;
    }
    ...
    const postResult = await client.query(
      `
      INSERT INTO posts
        (author_id, title, body, unit_name, locality, county, incident_date, status, display_name)
      VALUES
        ($1, $2, $3, $4, $5, $6, $7, $8, $9)
      RETURNING
        id, author_id, title, body, unit_name, locality, county,
        incident_date, status, display_name, created_at
      `,
      [
        req.userId,
        title || null,
    ...
    res.status(201).json({
      message:
        'Postare trimisă cu succes. Așteaptă aprobarea administratorului.',
      post: {
        id: post.id,
...
export const getPostById = async (
  ...
  try {
    ...
    const postResult = await pool.query(
      `
      SELECT
        id, title, body, unit_name, locality, county,
        incident_date, status, display_name, created_at
      FROM posts
    ...
    if (postResult.rows.length === 0) {
      res
        .status(404)
        .json({ error: 'Postare negăsită sau neaprobată.' });
      return;
    }
